...
cv_window_name = 'classifier_scalable - GoogleNet'

# read image files from this directory
...
def overlay_on_image(display_image:numpy.ndarray, label, prob, fps:float):
...
def handle_args():
    ...
    labels = GoogleNetProcessor.get_classification_labels()
...
def main():
    ...
    # Create a neural network processor list and an idle nn processor list
    # and store it in our list of processors
    nn_processor_list = list()
    idle_nn_processor_list = list()

    device_number = 0

    for one_device in devices:
        try:
            nn_proc_dev = mvnc.Device(one_device)
            nn_proc_dev.open()
            print("opened device " + str(device_number))
            obj_detector_proc = GoogleNetProcessor(NETWORK_GRAPH_FILENAME, nn_proc_dev,
                                                      name="classifier " + str(device_number))
            if (device_number < device_count):
                nn_processor_list.append(obj_detector_proc)
            else:
                idle_nn_processor_list.append(obj_detector_proc)
    ...
    if len(nn_processor_list) < 1:
        print('Could not open any NCS devices.')
        print('Reinsert devices and try again!')
        return 1

    print("Using " + str(len(nn_processor_list)) + " devices for classification")
    print_hot_keys()
    ...
    exit_app = False
    while (True):
        for one_nn_proc in nn_processor_list:
            print("using neural network processor: " + one_nn_proc.get_name())
            one_nn_proc.drain_queues()

        # image processor that will put images on the nn processor's input FIFO queue
        image_proc = ImageProcessor(input_image_path, one_nn_proc.get_nn_image_width(), one_nn_proc.get_nn_image_height(),
                                    GoogleNetProcessor.get_mean(),
                                    network_processor_list = nn_processor_list)
        image_proc.start_processing()
        ...
        while(True):
            ...
            for one_nn_proc in nn_processor_list:
                try:
                    (index, label, prob, display_image) = one_nn_proc.get_async_inference_result()
                except :
                ...
                if (prop_val < 0.0):
                    end_time = time.time()
                    image_proc.stop_processing()
                    image_proc.cleanup()
                    exit_app = True
                    break

                running_fps = frame_count / (time.time() - start_time)
                overlay_on_image(display_image, label, prob, running_fps)
            ...
            if ((time.time() - last_throttle_time) > throttle_check_seconds):
                #long time, check for throttling devices
                ...
                print("check and adjust for throttling")
                image_proc.pause()
                do_throttle_adjustment(nn_processor_list, idle_nn_processor_list)
                image_proc.unpause()
        ...
        for one_nn_proc in nn_processor_list:
            throttling = one_nn_proc.get_device().get_option(mvnc.DeviceOption.RO_THERMAL_THROTTLING_LEVEL)
            if (throttling > 0):
                print("\nDevice " + one_nn_proc.get_name() + " is throttling, level is: " + str(throttling))
                throttling_list.append(one_nn_proc)


        if (not exit_app):
            # rest between loops, display an image while resting
            ...
            if ((len(throttling_list) > len(idle_nn_processor_list))):
        ...
        for one_throttling in throttling_list:
            nn_processor_list.remove(one_throttling)
            nn_processor_list.append(one_throttling)


        num_idle = len(idle_nn_processor_list)
        if (num_idle > len(nn_processor_list)):
            num_idle = len(nn_processor_list)
        if (num_idle > 0):
            ...
            for idle_index in range(0, num_idle):
                ...
                nn_processor_list.insert(0, idle_nn_processor_list.pop(0))

            for idle_count in range(0, num_idle):
                idle_nn_processor_list.append(nn_processor_list.pop())
    ...
    for one_nn_proc in nn_processor_list:
        cv2.waitKey(1)
        one_nn_proc.cleanup(True)

    cv2.destroyAllWindows()


def do_throttle_adjustment(nn_processor_list, idle_nn_processor_list):
    throttling_list = list()
    for one_obj_detect_proc in nn_processor_list:
        throttling = one_obj_detect_proc.get_device().get_option(mvnc.DeviceOption.RO_THERMAL_THROTTLING_LEVEL)
        if (throttling > 0):
    ...
    for one_idle_proc in idle_nn_processor_list:
    ...
    if (num_idle > 0):
        ...
        for idle_index in range(0, num_idle):
            ...
            # for one_idle_proc in idle_obj_detect_list:
            the_idle_to_use = idle_nn_processor_list.pop(0)
            #print("moving idle : " + the_idle_to_use.get_name() + " to obj detect list")
            nn_processor_list.insert(0, the_idle_to_use)

        for idle_count in range(0, num_idle):
            #print("idle count is: " + str(idle_count))
            the_active_to_move = nn_processor_list.pop()
            #print("removing from active list: " + the_active_to_move.get_name())
            idle_nn_processor_list.append(the_active_to_move)
